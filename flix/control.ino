...
extern void updateAltHold();  // without remote control used python to hold position
...
void control() {
	...
	// add stable possition with out remote control 
	interpretControls();
    failsafe();
    updateAltHold();   // <--- add (sets thrustTarget & attitudeTarget in AUTO)
    controlAttitude();
    controlRates();
    controlTorque();
}
